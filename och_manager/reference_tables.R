...
manage.congregations.info = list(
  ...
  columns = data.frame(
    column.name = c("CongregationID", "CongregationName", "Street", "City",
                    ...
                    "Longitude", "FolderName", "UseData", "Created", "Updated"),
    key.table = rep(NA_character_, 14),
    key.label = rep(NA_character_, 14),
    width = c(150, 300, 200, 200, 50, 70, 200, 300, 100, 100, 300, 80, 100,
              100),
    type = c("numeric", "text", "text", "text", "text", "text", "text", "text",
             "numeric", "numeric", "text", "checkbox", "date", "date"),
    editable = c(F, T, T, T, T, T, T, T, F, F, F, T, F, F),
    html = rep(F, 14),
    stringsAsFactors = F
  ),
  fixed.cols = 2,
