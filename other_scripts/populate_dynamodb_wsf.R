...
song.instances.songbooks.df = dbGetQuery(wsdb.con,
  ...
  mutate(SongInstanceIDEntryNumber = paste(SongInstanceID, EntryNumber,
                                           sep = "-"))
write.to.dynamodb(song.instances.songbooks.df,
                  c("SongInstanceID", "SongID", "SongbookID",
                    "SongbookVolumeID"),
...
song.instances.artists.df = dbGetQuery(wsdb.con,
  ...
  mutate(SongInstanceIDRole = paste(SongInstanceID, Role, sep = "-")) %>%
  rename(ArtistRole = Role)
...
tunes.df = dbGetQuery(wsdb.con, "SELECT * FROM wsf.tunes") %>%
  mutate(RealTuneName = as.logical(RealTuneName))
...
lyrics.first.lines.df = dbGetQuery(wsdb.con,
  ...
  mutate(LyricsIDFirstLineOrder = paste(LyricsID, FirstLineOrder, sep = "-"))
