...
write.dynamo.items = function(db.con, dynamo.con, table.info, change.key) {
  if("dynamo.updates" %in% names(table.info)) {
    for(write.table in names(table.info$dynamo.updates$write)) {
      sql = glue_sql(table.info$dynamo.updates$write[[write.table]],
...
get.dynamo.delete.keys = function(db.con, table.info, temp.df, exclude = T) {
  if("dynamo.updates" %in% names(table.info) &
     "delete" %in% names(table.info$dynamo.updates)) {
    key = table.info$key
...
delete.dynamo.items = function(dynamo.con, table.info, delete.keys) {
  if("dynamo.updates" %in% names(table.info) &
    ...
    for(i in 1:nrow(delete.keys)) {
      ...
      key[[field]] = key.field
      for(table.name in table.info$dynamo.updates$delete) {
        dynamo.con$delete_item(Key = key, TableName = table.name)
      }
    }                    
  }
}
...
delete.dynamo.multi.items = function(db.con, dynamo.con, table.info, id) {
  if("dynamo.updates" %in% names(table.info) &
     "multi" %in% names(table.info$dynamo.updates)) {
    for(multi.table in names(table.info$dynamo.updates$multi)) {
      sql = table.info$dynamo.updates$multi[[multi.table]]$sql
      sql = gsub("SELECT +\\*",
                 paste("SELECT",
                       paste(table.info$dynamo.updates$multi[[multi.table]]$keys,
                             collapse = ", ")),
                 sql)
      ...
      if(nrow(delete.keys) > 0) {
        for(i in 1:nrow(delete.keys)) {
          item = list()
          for(field in table.info$dynamo.updates$multi[[multi.table]]$keys) {
...
insert.dynamo.multi.items = function(db.con, dynamo.con, table.info,
  ...
  if("dynamo.updates" %in% names(table.info) &
     "multi" %in% names(table.info$dynamo.updates)) {
    for(multi.table in names(table.info$dynamo.updates$multi)) {
      sql = glue_sql(table.info$dynamo.updates$multi[[multi.table]]$sql,
                     keys = change.key, .con = db.con)
      items.df = dbGetQuery(db.con, sql)
      if(nrow(items.df) > 0) {
        item.cols = sapply(items.df, class)
